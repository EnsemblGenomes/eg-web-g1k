package EnsEMBL::Web::Command::UserData::ForgeRun10;
...
sub process {
  ...
  foreach (qw(tmin tmax reps ctrls src bkgd)) {
      if ($hub->param($_)) {
  ...
  unless ($error) {
      ...
      $forge = Bio::Analysis::Forge10->new(
	  {
	      output => $fname,
	      bkgd => $hub->param('bkgd'),
	      data => $hub->param('src'),
  ...
  unless ($error) {
	  ...
	  $error .= " SNPs have been uploaded. Analysis requires at least 20 variations to work. ";
      }
  }
