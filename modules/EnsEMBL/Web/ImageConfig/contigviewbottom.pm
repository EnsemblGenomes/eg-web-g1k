...
#    [ 'variation_set_1kg_com','variation_set_ph_variants', 'sv_set_1kg_hq' ],
    [ 'variation_set_1kg_p3', 'variation_set_1kg_3' ],
    { display => 'compact' }
  );
...
sub _add_1kg_tracks {
  ...
  my @vcf_tracks  = split /,/, $self->hub->species_defs->DEFAULT_VCF_TRACKS;  
  my %default_tracks;
  foreach my $track (@vcf_tracks) {
    $default_tracks{$track} = 'histogram';
  }

  my $menu = $self->get_node( 'g1k' ) || return;

# move 1kg tracks into a separate menu
  my $h = $self->get_node( '1kg' ) || return;
  foreach my $n  (@{$h->child_nodes||[]}) {
      (my $cid = $n->id) =~ s/variation_set_1kg/kg_feature_variation_/;
      my $clone = $n->create_node($cid, $n->data);
      my $display = $default_tracks{$cid} || 'off';
      $clone->data->{display} = $display;
      $menu->append($clone);
      $n->remove();
  }
